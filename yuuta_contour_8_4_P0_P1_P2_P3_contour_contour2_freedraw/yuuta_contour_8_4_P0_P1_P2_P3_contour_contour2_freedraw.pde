...
void mouseDragged(){
  ...
  if(mouseButton==RIGHT){
    ...
    PVector oldV2 = new PVector(oldV.y, -oldV.x), newV2 = new PVector(newV.y, -newV.x);
    for(int i=0; i<contour.size(); i++){
      ...
      float ratio2 = PVector.dot(oldV2, oldvv)/ oldV2.magSq();
      println(ratio);
      contour2.get(i).x = P[2].x + ratio*newV.x + ratio2*newV2.x;
      contour2.get(i).y = P[2].y + ratio*newV.y + ratio2*newV2.y;
    }
  }
}
